...
from PortfolioProject.CovidDeaths
where SUBSTRING(date, -4) IN ('2020', '2021') and location like '%india%' -- continent = ''
group by Location
...
select Location, SUM(reproduction_rate)/COUNT(reproduction_rate) as AvgReproductionRate   
from PortfolioProject.CovidDeaths
where SUBSTRING(date, -4) IN ('2022', '2023', '2024') and location like '%india%' -- continent = ''
group by Location
order by AvgReproductionRate;
...
select Location, SUM(icu_patients)/COUNT(icu_patients) as AvgNoOfICUPatients    
from PortfolioProject.CovidDeaths
where SUBSTRING(date, -4) IN ('2020', '2021') and icu_patients != ''
group by Location
order by AvgNoOfICUPatients;
